...
function Set-CertificateValidationPolicy {
    # No longer needed in PowerShell 7+ as we can use -SkipCertificateCheck directly
    # This function remains for backwards compatibility but does nothing in PS7+
}
...
if ($null -eq $Connection) {
    throw "Connection object is null"
}

if ($null -eq $PendingChanges.ETag) {
    throw "ETag is null"
}
    $oneTimeConnection = Get-CMKConnection -Hostname $Connection.hostname -Sitename $Connection.sitename -Username $Connection.username -Secret $ConnSecret -IfMatch $PendingChanges.ETag[0]
    try {
...
function Rename-CMKHost {
    ...
    $ConnSecret = $Connection.Header.Authorization.Split(' ')[2] | ConvertTo-SecureString -AsPlainText -Force
    $etagValue = $HostObject.ETag[0] # Extract first element from ETag array
    $oneTimeConnection = Get-CMKConnection -Hostname $Connection.hostname -Sitename $Connection.sitename -Username $Connection.username -IfMatch $etagValue -Secret $ConnSecret
    $newName = @{
...
function Remove-CMKHostLabel {
    ...
    If ($HostObject.extensions.attributes.labels) {
        ...
        if ($Labels.ContainsKey($Key)) {
            ...
            Write-Verbose "Label $($Key) not found on host $($HostObject.id)"
            return $false
        }
    }
    else {

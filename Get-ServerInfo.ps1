        ...
        try {
            ...
            $ServerInfo = Invoke-Command -Session $Session -ScriptBlock {
                ...
                $allServices = Get-WmiObject Win32_Service | 
                    Select-Object DisplayName, StartMode, State, PathName
                ...
                [PSCustomObject]@{
                    ...
                    DiskInfo = $diskInfo
                    Services = $allServices # Added Services info
                }
            }
...
SERVICES
==================================================
$($ServerInfo.Services | Sort-Object DisplayName | ForEach-Object {
    "Display Name:   $($_.DisplayName)`n" +
    "Startup Type:   $($_.StartMode)`n" +
    "Status:         $($_.State)`n" +
    "Executable:     $($_.PathName)`n"
} | Out-String)

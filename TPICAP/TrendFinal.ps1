...
foreach ($server in $servers) {
    ...
    if ($sessionOrError -is [System.Management.Automation.Runspaces.PSSession]) {
        ...
        try {
            $serviceList = Invoke-Command -Session $session -ScriptBlock { Get-Service | Where-Object { $_.DisplayName -like "*Trend*" } } -ErrorAction SilentlyContinue
            if ($serviceList) {
                $containsTrendMicroDSM = ($serviceList | Where-Object { $_.Name -eq 'ds_monitor' } | Measure-Object).Count -gt 0
                if ($containsTrendMicroDSM) {
                    $trendMicroOutput = Invoke-Command -Session $session -ScriptBlock { & "C:\Program Files\Trend Micro\Deep Security Agent\dsa_control.cmd" -m }
                } else {
                    $trendMicroOutput = "Trend Micro DSM not found"
                }
            } else {
                $errorDetails = "Failed to query services"
                $containsTrendMicroDSM = $false
                $trendMicroOutput = "Failed to query Trend Micro DSM"
            }

            # Test-NetConnection logic integrated here
            $testUrls = @("app.deepsecurity.trendmicro.com", "agents.workload.gb-1.cloudone.trendmicro.com", "dsmim.deepsecurity.trendmicro.com")
    ...
    $results += [PSCustomObject]@{
        ...
        TrendMicroAgent = $containsTrendMicroDSM
        TrendMicroOutput = $trendMicroOutput

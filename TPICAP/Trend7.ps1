...
function Check-Trendapps ($session) {
    ...
    $subKeys += $subKeysWow6432Node
    $apps = @() # Initialize an empty array
    
    foreach ($subKey in $subKeys) {
        $displayName = $subKey.GetValue("DisplayName")
        $displayVersion = $subKey.GetValue("DisplayVersion") # Get the version
        
        if ($displayName -like "*Trend*") {
            Write-Host "$env:COMPUTERNAME has $displayName version $displayVersion"
            $appInfo = [PSCustomObject]@{
                Name = $displayName
                Version = $displayVersion
            }
            $apps += $appInfo # Append to array
        }
    }
    
    # Cleaned-up output
    Write-Host "Cleaned-up output:"
    foreach ($app in $apps) {
        Write-Host ("{0} (Version: {1})" -f $app.Name, $app.Version)
    }
...
function Get-TrendServices {
    ...
    if ($services) {
        foreach ($service in $services) {
            $output += "$($service.DisplayName): $($service.Status)"
        }
    } else {
        $output += "None: None"
    }
...
foreach ($server in $servers) {
  ...
  $trendaddp = Check-Trendapps -session $session

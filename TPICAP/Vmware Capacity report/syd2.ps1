...
# Input required for this script: vCenter server IP/hostname, vCenter server credentials, SMTP server IP/hostname
...
$vCenterServerName = "syd2va0001" 
$ToAddress = "dennisjeffrey.flores@tpicap.com", "brad.craig@tpicap.com", "Carlos.Quiroz@tpicap.com>", "ealabre@liquidnet.com", "Ian.Navarrete@tpicap.com"
#-----------------------------------------------------------------------------
...
$FromAddress = "VMwareEMEACapacity@tpicap.com"
...
Connect-VIServer -Server $vCenterServerName -Username corp\srvcDev42VC -Password "R#2TwaM@" -WarningAction SilentlyContinue -Force
...
$Snapshots = Get-VM | Get-Snapshot | Select VM,Name,@{Label=”Size”;Expression={“{0:N2} GB” -f ($_.SizeGB)}},Created
...
$HTMLBody = "<BODY>"
$HTMLBody += "Report Generated on:$Date<br>"
$HTMLBody +=
...
$HTMLPage | Out-File ((Get-Location).Path + "\capacityreport.html") # Export locally to html file
Write-Host "Report has exported to HTML file " + ((Get-Location).Path + "\capacityreport.html")
...
Disconnect-VIServer -Server LD5PINFVCA01 -confirm:$false
